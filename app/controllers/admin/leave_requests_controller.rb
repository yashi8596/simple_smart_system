class Admin::LeaveRequestsController < Admin::Base
  ...
  def update
    ...
    if @request.update(request_params)
      ...
      if @request.permitted == true
        #申請ステータスが更新された場合の処理
        #eventテーブルに記録
        event = EmployeeEvent.new
        event.employee_id = @request.employee_number
        event.paid_leave = @request.preferred_date
        event.type = "有給"
        ...
        if event.save && @employee.update_attribute(:number_of_paid_leave, paid_leave)
          ...
          redirect_to admin_employee_path(@employee.id) and return
        end

      elsif @request.canceled == true
        #取消ステータスが更新された場合の処理
        #eventsテーブルから該当データを探す
        event = EmployeeEvent.find_by(employee_id: @request.employee_number)
